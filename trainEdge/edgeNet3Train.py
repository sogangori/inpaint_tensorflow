...
NUM_EPOCHS =3
EVAL_BATCH_SIZE = 16
trainDataCount = 1024 * 64
EVAL_FREQUENCY = trainDataCount / EVAL_BATCH_SIZE 
FLAGS = tf.app.flags.FLAGS
isNewTrain = not True
startLearningRate = 0.0005

def main(argv=None):  # pylint: disable=unused-argument
      
  trainIn, trainOut = model.GetTrainInput(trainDataCount) 
  ...
  #loss = model.getLoss_center(train_prediction, train_labels_node)
  ...
  def eval_in_batches(data, sess):
    ...
    predictions = numpy.ndarray(shape=(size, model.NUM_LABELS), dtype=numpy.float32)
