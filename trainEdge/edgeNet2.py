...
modelName = "../weights/edgeConv4.pd"
logName ="../weights/logs_edgeConv4"
...
#conv4_weights= tf.Variable(    tf.truncated_normal([conv4_kernelWidth, conv4_kernelWidth, conv3_weightCount, conv4_weightCount],stddev=0.1,seed=SEED),name="conv4_w")
...
def inference(variableDic, train=False):   
    ...
    relu = tf.nn.sigmoid(tf.nn.bias_add(conv, variableDic["conv3_biases"]))
...
def getLoss_hint(prediction,labels_node,train_hint_node):
    ...
    loss = tf.reduce_mean(tf.square(diffNode))                                        
                    
    lossEdge = tf.reduce_mean(tf.square((prediction-labels_node)*train_hint_node))
    loss = (loss+lossEdge*3)/2;
    return loss
...
def GetTrainInput(trainCount):    
    ...
    [labelSet,DamagedSet,labelEdgeSet] = trainSetMaker.generatePatchSetWidthHint(imageFile, trainCount, IMAGE_SIZE)    
    return [DamagedSet, labelSet,labelEdgeSet]
