...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  train_prediction = inpaintPatch.inference(train_data_node, True)
  eval_prediction = inpaintPatch.inference(eval_data, True)
      
  cross_entropy = -tf.reduce_sum(train_prediction-train_labels_node)
  loss1 = tf.reduce_mean(tf.square(train_prediction-train_labels_node))
  loss2 = tf.reduce_mean(tf.square(train_prediction[:,9*9*(inpaintPatch.NUM_CHANNELS_Out-1):]-train_labels_node[:,9*9*(inpaintPatch.NUM_CHANNELS_Out-1):]))
  loss = loss1+loss2*2
